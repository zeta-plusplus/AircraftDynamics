...
model PtMassFrame
  ...
  /********************************************************
    ...
    ********************************************************/
  //********** Parameters **********
  parameter Integer nIn_Fz = 1 "Number of Fz inputs" annotation(
    Dialog(group = "Port setting"));
  parameter Integer nIn_FxForward = 1 "Number of thrust inputs" annotation(
    Dialog(group = "Port setting"));
  parameter Integer nIn_FxBackward = 1 "Number of drag inputs" annotation(
    Dialog(group = "Port setting"));
  parameter Integer nIn_mass = 0 "Number of mass(weight) inputs" annotation(
    Dialog(group = "Port setting"));
  //********** Internal variables **********
...
protected
  ...
  //********** objects **********
  AircraftDynamics.Records.FlightStates fltStates(V.start = Vinit, alpha.start = alphaInit, gamma.start = gammaInit, q.start = qInit);
