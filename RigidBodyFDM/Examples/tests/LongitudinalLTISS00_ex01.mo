...
model LongitudinalLTISS00_ex01
  ...
  Modelica.Blocks.Math.UnitConversions.From_deg from_deg3 annotation(
    Placement(visible = true, transformation(origin = {-60, -30}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  AircraftDynamics.Aerodynamics.Components.Flight2Fluid flight2fluid(redeclare package Medium = surrFluid, MNdes = 0.5, altDes = 0)  annotation(
...
equation
  ...
  connect(LongiSS.par_dynP1, flight2fluid.y_dynP) annotation(
    Line(points = {{-12, 22}, {-12, 62}, {-59, 62}}, color = {0, 0, 127}));
  connect(LongiSS.par_U1, flight2fluid.y_V_tot) annotation(
    Line(points = {{-3, 22}, {-3, 66}, {-59, 66}}, color = {0, 0, 127}));
