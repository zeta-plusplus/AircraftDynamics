...
model LongitudinalLTISS00_ex01
  ...
  Modelica.Blocks.Sources.RealExpression uSignal_deltaE(y = if time <= 2 then 0 elseif 2 <= time and time <= 3 then -3 elseif 30 <= time and time <= 31 then -3    elseif 31 <= time then -3 else -3)  annotation(
    Placement(visible = true, transformation(origin = {-90, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
...
equation
  ...
  connect(const_m.y, FltDynLongiSS.par_m) annotation(
    Line(points = {{40, 100}, {40, 42}}, color = {0, 0, 127}));
  connect(const_S.y, FltDynLongiSS.par_S) annotation(
    Line(points = {{10, 100}, {10, 90}, {32, 90}, {32, 42}}, color = {0, 0, 127}));
  connect(FltDynLongiSS.par_U1, Flt2Fluid.y_V_inf) annotation(
    Line(points = {{12, 42}, {14, 42}, {14, 56}, {-58, 56}}, color = {0, 0, 127}));
  connect(Flt2Fluid.y_qBar, FltDynLongiSS.par_q1bar) annotation(
