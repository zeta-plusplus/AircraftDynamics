...
model visAtt_LongitudinalLTISS00_ex01
  ...
  Modelica.Blocks.Sources.RealExpression uSignal_deltaE(y = if time <= 2 then 0 elseif 2 <= time and time <= 3 then -3
   ...
   elseif 31 <= time then 0 else 0) annotation(
    Placement(visible = true, transformation(origin = {-90, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Sources.RealExpression uSignal_deltaFracT(y = if time <= 250 then 0 elseif 250 <= time and time <= 300 then 0.2
   elseif 350 <= time and time <= 400 then 0
...
equation
  connect(FltDynLongiSS.VisInfoOut, AnimAircraft.VisInfoIn) annotation(
    Line(points = {{46, 0}, {46, 0}, {46, -50}, {60, -50}, {60, -50}}, color = {95, 95, 95}));
...
end visAtt_LongitudinalLTISS00_ex01;
